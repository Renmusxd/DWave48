...
if [ -z "$BASE_DIR" ]; then
	...
	echo "qsub -pe omp \"$NUM_ANALYZE_SLOTS\" -wd \"$BASE_DIR/code\" -t 1-$NUM_SHARDS run.sh \"$BASE_DIR\" \"$NUM_SHARDS\""
	OUTPUT=$(qsub -pe omp "$NUM_ANALYZE_SLOTS" -wd "$BASE_DIR/code" -t 1-"$NUM_SHARDS" run.sh "$BASE_DIR" "$NUM_SHARDS")
	echo "$OUTPUT"
