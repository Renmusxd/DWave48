...
def make_edges_svg(edges, unit_cells_per_row=16, vars_per_cell=8, dist=5, color_fn=None):
    contents, _ = make_edges_contents(edges, unit_cells_per_row=unit_cells_per_row, vars_per_cell=vars_per_cell,
                                      dist=dist, color_fn=color_fn)
    return'<svg viewBox="0 0 1 1" preserveaspectration="none" xmlns="http://www.w3.org/2000/svg">\n{}</svg>'.format(
        contents
    )


def make_edges_contents(edges, unit_cells_per_row=16, vars_per_cell=8, dist=5, color_fn=None):
    ...
    for to_draw, to_detail in zip(to_draw_values, debug_details_values):
        ...
        output.write("<!-- {} -->\n".format(title_text))
        output.write('<line x1="{}" y1="{}" x2="{}" y2="{}" style="stroke:{};stroke-width:0.01"><title>{}</title></line>\n'.format(
...
def make_dimer_contents(broken_edges, normalize, unit_cells_per_row=16, vars_per_cell=8, dist=5,
    ...
    for (start_x, start_y), (stop_x, stop_y), (var_a, var_b) in dimer_positions:
        ...
        output.write('<line x1="{}" y1="{}" x2="{}" y2="{}" style="stroke:{};stroke-width:{}"><title>{}</title></line>\n'.format(
...
def make_dimer_svg(js, var_vals, unit_cells_per_row=16, vars_per_cell=8, dist=5,
    ...
    background_dimer_contents = make_dimer_contents(edges, normalize, unit_cells_per_row=unit_cells_per_row,
                                                    vars_per_cell=vars_per_cell, dist=dist,
                                                    dimer_color=dimer_edge_color, width="0.005")
    dimer_contents = make_dimer_contents(broken_edges, normalize, unit_cells_per_row=unit_cells_per_row,
                                         vars_per_cell=vars_per_cell, dist=dist, dimer_color=dimer_color)
    return '<svg viewBox="0 0 1 1" preserveaspectration="none" xmlns="http://www.w3.org/2000/svg">\n' \
           '{}\n' \
           '{}\n' \
           '{}' \
           '</svg>'.format(background_edges_contents, background_dimer_contents, dimer_contents)
